...
function tree(schema, options) {
     ...
     * @return {Model}
     */
    schema.statics.getChildren = function getChildren(root, filters, fields, options, recursive, next) {
        ...
        if (recursive) {
            if(filters['$query']){
                filters['$query']['path'] = {$regex: '^' + root.path + pathSeparatorRegex};
            } else {
                filters['path'] = {$regex: '^' + root.path + pathSeparatorRegex};
            }
        } else {
            if(filters['$query']){
                filters['$query']['parent'] = root._id;
            } else {
                filters['parent'] = root._id;
            }
        }

        return this.find(filters, fields, options, next);
    };
    ...
    schema.methods.getChildren = function getChildren(filters, fields, options, recursive, next) {
        return schema.statics.getChildren(this, filters, fields, options, recursive, next);
    };
